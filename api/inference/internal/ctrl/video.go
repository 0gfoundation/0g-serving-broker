...
func (c *Ctrl) videoReserveUnitsFromRequest(reqBody []byte, contentType string) (int64, error) {
	...
	if entry != nil && entry.Billing != nil && !entry.Billing.HasResolution(size) {
		...
		} else if mx := entry.Billing.MaxResolutionMultiplier(); mx > 1 {
			if dearest := videoOutputCount(seconds, mx); dearest > lifted {
				lifted = dearest
			}
		}
		units = lifted
...
//
// Compares UNITS, not fees. The gate priced its units at GetCachedService's output price — the
// service ceiling over all models — while settlement bills at the per-model price, so on a
// heterogeneous service a units shortfall can be fully absorbed by that headroom. Direction is still
// safe (fewer units can never cost more), so this over-reports rather than under-reports; the Help
// text says so, and the RATE is the signal rather than any single increment.
//
// Deliberately observability-only. It cannot refuse anything: the video is already generated by the
// time settlement runs, and refusing to bill would serve it free.
func (c *Ctrl) checkVideoReserveCoverage(reqBody []byte, contentType string, settledUnits int64) {
	...
	if err != nil {
		// The recompute reads CURRENT config while the gate ran under whatever config was live at
		// create time — poll jobs persist their request body across restarts — so an error here means
		// the reserve's model of this request changed in between. That is the drift class this whole
		// check exists to surface, so it is counted rather than swallowed: returning quietly turned
		// the detector off in precisely the scenario it was built for.
		monitor.RecordVideoReserveShortfallUnknown()
		c.logProofSkip("video_reserve_recompute_failed", "",
			"video settlement could not recompute the pre-flight reserve for a %d-unit bill (config drifted since create time?): %v",
			settledUnits, err)
		return
	}
	if reserved >= settledUnits {
		return
	}
	monitor.RecordVideoReserveShortfall()
	// Empty detail, deliberately. logProofSkip keys its throttle memo on (reason, detail), and its
	// contract is that the detail never comes from the request — `settledUnits` is
	// ceil(seconds x multiplier), so a caller varying `seconds` mints a fresh key per request: one
	// ERROR line each, and past maxProofSkipKeys the overflow handler flushes the whole memo, which
	// un-throttles the routing-proof reasons for everyone. That is the #542 log-amplification shape,
	// and the sibling per_unit_table_uncovered call passes "" for exactly this reason. Both numbers
	// live in the message; the rate lives in the counter.
	c.logProofSkip("video_reserve_shortfall", "",
		"video settlement billed %d units against a %d-unit pre-flight reserve: the reserve's model of this request was short. Units, not fees — the gate priced at the service ceiling, so this is not by itself proof the balance gate was inadequate",
