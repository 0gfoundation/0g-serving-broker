...
// reserveInFlightVideoFee stamps the pre-flight reserve (computed at the gate by
// VideoCreateReserve) onto this request's row, so the job counts against the
// wallet's balance for as long as it is in flight.
//
