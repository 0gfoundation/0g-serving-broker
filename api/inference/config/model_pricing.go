...
// this model prices NOWHERE — in practice any pixel-dimension size, because no live translator lets
// the client's pixel dimensions name the tier. MiniMax uses them for the ASPECT RATIO only and renders
// MINIMAX_RESOLUTION from its own environment (`minimax.go`: "deriving a tier from pixels would
// produce a value H3 rejects"); DashScope derives the tier from `max(width, height) > 1280`, not from
// either dimension alone. So the vendor picks the tier, not the client, and the only reserve that is a
// true ceiling is the dearest the model can bill. Without it the reserve sat at the 1.0 baseline and
// settled at whichever tier came back — `size:"1920x1080"` against `{720p:1.0, 1080p:1.5}` reserved 5
// and billed 8.
//
// Deriving the tier from the pixel height instead was tried and reverted: it made the reserve agree
// with NEITHER translator, so `size:"1280x720"` reserved the 720p row against a 2K render (MiniMax)
// and `size:"1920x720"` reserved 720p against DashScope's max-side 1080P. Trading a known
// over-reserve for an under-reserve is trading a refused request for a free one.
//
