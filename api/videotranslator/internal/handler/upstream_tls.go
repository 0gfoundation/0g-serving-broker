...
// (c.JSON / c.Status + io.Copy) after the vendor call returns, and a header set
// after the status line is written is dropped.
...
// engine can flush c.writermem directly, bypassing this wrapper entirely. Every
// current handler writes a body, so that path is unreachable here.
...
// no report. No sidecar handler streams today (GetVideoContent io.Copy's, which
// goes through Write), so this closes the gap for the streaming handler someone
// adds later rather than a live one.
