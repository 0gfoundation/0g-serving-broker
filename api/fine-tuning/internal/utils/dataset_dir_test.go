...
// The resolver takes the HASH, and these are the two cases that prove why. Resolving the
// DIRECTORY alone was the first version of this, and it broke in both: a directory existing
// says nothing about whether the dataset being asked for is in it.
func TestResolveDatasetPathResolvesAgainstTheDatasetWanted(t *testing.T) {
	t.Run("an empty canonical directory does not hide a legacy dataset", func(t *testing.T) {
		...
		if got := ResolveDatasetPath(spellingLower, "hash1"); got != legacy {
	...
	t.Run("two legacy directories are each found for what they hold", func(t *testing.T) {
		...
		for _, tc := range []struct {
			...
			for _, asked := range []string{spellingEIP55, spellingLower} {
				if got := ResolveDatasetPath(asked, tc.hash); got != tc.want {
	...
	t.Run("the pre-converted HF form counts as present", func(t *testing.T) {
		...
		SetDataDir(t.TempDir())
		seedDataset(t, spellingEIP55, "hash1_hf")
		want := filepath.Join(GetDataDir(), "datasets", spellingEIP55, "hash1")
		if got := ResolveDatasetPath(spellingLower, "hash1"); got != want {
	...
	t.Run("a dataset in the canonical directory is preferred", func(t *testing.T) {
		...
		canonicalName := filepath.Base(DatasetDir(spellingLower))
		inCanonical := seedDataset(t, canonicalName, "hash1")
		if got := ResolveDatasetPath(spellingEIP55, "hash1"); got != inCanonical {
	...
	t.Run("a folded directory that does not hold the hash is skipped", func(t *testing.T) {
		...
		//
		// The earlier version of this file missed that: its two-directory case happened to
	...
	t.Run("nothing anywhere names the canonical path", func(t *testing.T) {
		...
		if got := ResolveDatasetPath(spellingEIP55, "hash1"); got != want {
...
func TestResolveDatasetPathIgnoresDirectoriesThatAreNotAddresses(t *testing.T) {
	...
	if got, want := ResolveDatasetPath(zero, "hash1"), filepath.Join(DatasetDir(zero), "hash1"); got != want {
...
func TestResolveDatasetPathIgnoresFiles(t *testing.T) {
	...
	if got, want := ResolveDatasetPath(spellingEIP55, "hash1"), filepath.Join(DatasetDir(spellingEIP55), "hash1"); got != want {
