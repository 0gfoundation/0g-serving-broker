...
type Service struct {
	...
	// DatasetQuotaPerUser is the maximum total storage (in bytes) each user may consume
	// for uploaded datasets under {dataDir}/datasets/{userAddress}/.
...
func GetConfig() *Config {
	once.Do(func() {
		instance = &Config{
			...
			Service: Service{
				DatasetQuotaPerUser: 10 * 1024 * 1024 * 1024, // 10 GB
			},
		}
