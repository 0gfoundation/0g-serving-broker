...
|---|---|
| `broker_requests_total` | the BOUNDED `CtxKeyMetricModel` (stamped by `PrepareHTTPRequest` via `ctrl.metricModel`; never the raw `CtxKeyResolvedModel`, which carries arbitrary user strings on wildcard deployments). Non-billed proxied endpoints (status polls, signature fetch) carry the configured model; "" remains on paths that never reach `PrepareHTTPRequest` and on requests rejected before resolution (allowlist rejections land in `model=""` with status>=400) |
