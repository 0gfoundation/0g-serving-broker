...
- **Biased shallow.** Reasoning tokens are billed as output, so an
  unrepresentable request resolves *down* whenever the template offers anything
  at or below it, never charging for depth nobody asked for. It resolves up only
  when the request is shallower than every declared level — and there the
  alternative, writing nothing, would leave the upstream's own default, which is
  deeper still: on GLM-5.2's `high`/`max`, a client asking for `low` is better
  served by `high` than by the default `max`.
